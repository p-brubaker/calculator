// import functions
import { add, subtract, multiply, divide, intdiv, pythag, hail, mod } from './operations.js';
...
const inputModX = document.getElementById('input-mod-x');
const inputModY = document.getElementById('input-mod-y');
...
btnMod.addEventListener('click', () => {
  const x = Number(inputModX.value);
  const y = Number(inputModY.value);
// do any needed work with the value(s)
  const result = mod(x, y);
// update DOM to reflect new value(s)
  resultMod.textContent = result;
})
